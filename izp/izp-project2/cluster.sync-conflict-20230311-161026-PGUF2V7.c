...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    if (is_string_number(count_token) == 1) {
            cluster_amount = atoi(count_token);
    } else {
        fprintf(stderr, "Invalid format of object count.\n");
        fclose(fp);
        return 0;
    }

    if (cluster_amount < 1) {
        fprintf(stderr, "Invalid value for the cluster amount in the input file.\n");
        fclose(fp);
        return 0;
    }
    ...
    *arr = malloc(cluster_amount * sizeof(struct cluster_t));
    ...
    for (int i = 0; i < cluster_amount; i++) {
        ...
        if (feof(fp) != 0 && i == cluster_amount + 1) {
            ...
            fprintf(stderr, "Could not read data from the input file.\n");
            fclose(fp);
            return 0;
        }
        ...
        if (is_string_number(x_token) == 0 || is_string_number(y_token) == 0) {
            fprintf(stderr, "Invalid coordinate format.\n");
            fclose(fp);
            return 0;
        } else if (obj.x < 0 || obj.x > 1000 || obj.y < 0 || obj.y > 1000) {
            fprintf(stderr, "Coordinates out of allowed range.\n");
            fclose(fp);
            return 0;
        } else if(strtok(NULL, " ") != NULL) {
            fprintf(stderr, "Invalid object definition or several objects on one line.\n");
            fclose(fp);
            return 0;
        }
        ...
        for (int j = 0; j < i; j++) {
            if ((*arr)[j].obj->id == (*arr)[i].obj->id && i != j) {
                fprintf(stderr, "IDs are not unique.\n");
