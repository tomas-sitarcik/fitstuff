...
void find_neighbours(struct cluster_t *carr, int narr, int *c1, int *c2)
{
    ...
    float dist = 1000; // maximum possible distance

    for (int i = 0; i < narr; i++) {
        for (int k = 0; k < carr[i].size; k++) {
            if (cluster_distance(&(carr[i]), &(carr[k])) < dist) {
                *c1 = i;
                *c2 = k;
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    load_clusters("./objects.txt", &clusters);

    print_clusters(clusters, 20);
    
    remove_cluster(clusters, 20, 4);
    print_clusters(clusters, 19);

    printf("%f\n", obj_distance(clusters[0].obj, clusters[1].obj));
    cluster_distance(&clusters[0], &clusters[1]);

    int c1;
    int c2;

    find_neighbours(clusters, 19, &c1, &c2);
