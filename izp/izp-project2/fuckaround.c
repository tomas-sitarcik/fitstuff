...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    for (int i = 0; i < c1->size; i++) {
        for (int k = 0; k < c2->size; k++) {
            float temp_dist = obj_distance(&(c1->obj[i]), &(c2->obj[k]));
            if (temp_dist < dist) {
                dist = temp_dist;
            }
        }
...
void find_neighbours(struct cluster_t *carr, int narr, int *c1, int *c2)
{
    ...
    float dist = 1000.0f; // maximum possible distance

    for (int i = 0; i < narr; i++) {
        for (int k = 0; k < narr; k++) {
            if (cluster_distance(&(carr[i]), &(carr[k])) < dist && i != k) {
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    *arr = malloc(amount * sizeof(struct cluster_t));
    ...
    for (int i = 0; i < amount; i++) {
        ...
        res = fscanf(fp, "%d %f %f", &obj.id, &obj.x, &obj.y);
...
/*
 Rozrazeni pole clusteru obj do N clusterů, podle Euklidovske vzdalenosti a vraci vyslednou
 velikost carr.
